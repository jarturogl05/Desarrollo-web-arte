...
const getPostsByAutor = async (req, res) => {
    ...
    const posts = await Post.paginate({autorId : ID},{limit:6, page:numberPage, sort:{_id: -1, createdAt: -1}});
...
const getPostsList = async(req, res) =>{
    const numberPage = req.params.page
    const posts = await Post.paginate({},{limit:6, page:numberPage, sort:{_id: -1, createdAt: -1}  });
    res.status(200).send(posts);
}
